...
namespace SchoolScore.Api.DACs
{
    ...
    {
        Task<IEnumerable<Models.StudentInClassroom>> ListWithStudent(string classroomId, IMongoCollection<Student> studentCollection, int page = 1, int? pageSize = null);
        Task<IEnumerable<Models.ClassroomStudent>> ListWithClassroomAndStudent(IMongoCollection<Classroom> classroomCollection, IMongoCollection<Student> studentCollection, Expression<Func<ClassroomStudent, bool>> expression, int page = 1, int? pageSize = null);
